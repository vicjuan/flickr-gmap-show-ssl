...
var google_lang='en';
var regexp_google_map_lang = /^http:\/\/www\.google\.com\/intl\/([a-zA-Z0-9\-]*)_ALL\/mapfiles(\/.*)?$/;
...
document.write = function(str) {
  if(str.indexOf('<script ')>=0){
    ...
    var rexp = regexp_google_map_lang.exec(src);
    if(rexp && rexp[1]) google_lang = rexp[1];
...
search:function(obj,callback,str,hl) {
  var cb='geocode_cb_'+new Date().getTime();
  FGS.geocode[cb]=function(rsp){arguments.callee.callback.call(obj,rsp);};
  FGS.geocode[cb].callback=callback;
  var path = 'http://pipes.yahoo.com/pipes/pipe.run?_id=DGxv33Y_3BGefeskouNLYQ&_render=json&_callback=FGS.geocode.'+cb+'&hl='+hl+'&q='+encodeURI(str);
  loadscript(path);
}
};
...
create_mapwindow.f_search_submit_callback=function(rsp){
  var issuccess = false;
  if(rsp.count == 1) {
    var gres = rsp.value.items[0];
...
create_mapwindow.f_search_submit=function(){
  try {
    geocode.search(this.win,this.win.f_search_submit_callback,this.s_str.value,google_lang);
  } finally {
    return false;
  }
};
